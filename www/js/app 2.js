  ...
  methods: {
    checkin: function () {
      ...
      navigator.camera.getPicture(function(fileURI){
        ...
        ft.upload(fileURI, encodeURI("http://lavie.liveapps.top/index.php/app/checkin"), function(raw){
          app.dialog.close();
    ...
    checkout: function () {
      ...
      app.request.post('http://lavie.liveapps.top/index.php/app/checkout/', { token:token }, function (raw) {
        if(raw=='invalid'){
          app.dialog.alert('Phiên làm việc đã hết hạn', 'Báo lỗi', function(){
            router.navigate('/login/', {reloadAll:true, ignoreCache:true});
          });
...
//preload some data
app.request.post('http://lavie.liveapps.top/index.php/app/allproducts', {}, function (raw) {
  try{
    var json = JSON.parse(raw);
